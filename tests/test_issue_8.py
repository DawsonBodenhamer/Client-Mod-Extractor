...
class Issue8RegressionTests(unittest.TestCase):
    ...
    @unittest.skipUnless(os.environ.get("CME_ISSUE8_ARTIFACTS"), "exact issue artifacts not supplied")
    def test_exact_downloaded_issue_artifacts(self):
        artifact_root = Path(os.environ["CME_ISSUE8_ARTIFACTS"])
        run_root = Path(tempfile.mkdtemp(prefix="exact-", dir=self.temp_root))
        for artifact in artifact_root.glob("*.jar"):
            shutil.copy2(artifact, run_root / artifact.name)

        FixtureHandler.payloads = {
            "/cf.json": json.dumps({"globalExcludes": ["appleskin"], "globalForceIncludes": []}),
            "/modrinth.json": json.dumps({"globalExcludes": [], "globalForceIncludes": []}),
            "/custom.txt": (
                "coolrain,eg_particle_interactions,norealmsbutton,windy,"
                "+appleskin,neoforge:advancementplaques"
            ),
            "/latest.json": json.dumps({"tag_name": "v1.0.8"}),
        }
        ...
        copied = {path.name for path in (run_root / "Save_For_Server_Mods").glob("*.jar")}
        self.assertEqual(copied, {"appleskin-neoforge.jar", "appleskin-fabric.jar"})
        self.assertIn("appleskin-neoforge.jar (force-included)", result.stdout)
        self.assertIn("appleskin-fabric.jar (force-included)", result.stdout)
        for filename in (
            "coolrain-neoforge.jar",
            "particle-neoforge.jar",
            "particle-fabric.jar",
            "norealmsbutton-neoforge.jar",
            "norealmsbutton-fabric.jar",
            "windy-neoforge.jar",
        ):
            self.assertIn(f"{filename} (mislabeled)", result.stdout)
        for filename in ("particle-fabric-patch.1.jar", "windy-fabric.jar"):
            self.assertIn(filename, result.stdout)
            self.assertNotIn(filename, copied)
        self.assertNotIn("coolrain-neoforge.jar", copied)
        self.assertIn("mislabeled", result.stdout)
